...
#pragma omp parallel for schedule(guided)
    ...
    RBF_kernel(const SyncArray<int> &self_dot0_idx, const SyncArray<kernel_type> &self_dot1,
        ...
        const int *self_dot0_data = self_dot0_idx.host_data();
        const kernel_type *self_dot1_data = self_dot1.host_data();
...
#pragma omp parallel for num_threads(64)
        for (int i=0;i<m;i++){
            ...
            for (int j=0;j<n;j++){
                ...
                for (int kk=row_begin;kk<row_end;kk++){
                    // sparseMat[k] // sparse[i][col_index_data[k]]
        ...
        //for (int i=0;i<m;i++){
        //    for (int j=0;j<n;j++){
        //        result_data[i*n+j] ref_data[i+j*m];
        //    }
        //}
